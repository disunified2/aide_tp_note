//
// Created by Théo Delaroche on 21/10/2025.
//
...
int main(int argc, char *argv[]) {
    ...
    // Actions à faire pour le serveur : socket, bind, listen, accept, send/recv, shutdown/close
    ...
    ssize_t byte_sent = send(client_sock, buffer, BUFSIZE, 0);
    if (byte_sent == -1) {
        perror("recv");
        close(client_sock);
        close(sockfd);
